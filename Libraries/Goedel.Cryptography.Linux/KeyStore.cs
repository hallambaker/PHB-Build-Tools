...
namespace Goedel.Cryptography.Linux {
    class KeyStore {
        ...
        // Feature: At present only DH keys are stored in the Linux keystore.
        // Feature: The linux keys should be secured using OpenPGP agent.

        readonly static byte[] EntropyBytes = "Mathematical Mesh Encrypted Key".ToBytes();
        const string DirectoryUserPrivate = @".Mesh/Keystore";
        ...
        public static void WriteToKeyStore(IPKIXPrivateKey Data, KeySecurity KeySecurity) {
            ...
            var Container = new KeyContainer(JoseKey, KeySecurity);
            var Plaintext = Container.ToJson(true);
            var CipherText = Plaintext; // DPAPI.Encrypt(Plaintext, Description: Description, EntropyBytes: EntropyBytes);
