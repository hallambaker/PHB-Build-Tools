...
namespace Goedel.DNS {
    ...
    public class DNSBufferIndex {
        ...
        void CheckSpaceWrite (int bytes) {
            Assert.False (( Pointer + bytes ) > Buffer.MaxLength, DNSBufferOverflow.Throw);
        ...
        void CheckSpaceRead(int bytes) {
            Assert.False(Pointer + bytes > MaxRead, DNSReadTruncated.Throw);
            }
        ...
        public void WriteIPv4 (IPAddress data) {
            ...
            Assert.True(bytes.Length == 4, DNSInvalidAddress.Throw);
            WriteData (bytes);
            }

        public void WriteIPv6 (IPAddress data) {
            byte [] bytes = data.GetAddressBytes();
            Assert.True(bytes.Length == 16, DNSInvalidAddress.Throw);
            WriteData (bytes);
            }
            ...
            for (int i=0; i <= Name.Length; i++) {
                ...
                if (n == '.') {
                    if (label != 0) { //Ignore zero length labels
                        Assert.False(label > 63, DNSLabelTooLong.Throw);
                ...
                else if (n == '-' | ( nn >= 65 & nn <= 90 ) | ( nn >= 48 & nn <= 57 )) {
                    WriteByte ((byte)nn);
                    label++;
                    }
                    ...
                    throw new NYI ("Unicode Not Supported Yet");
                    }
                else throw new DNSIllegalCharacter("Illegal character");
                }
            }
        public void WriteTag (String Tag) {
            CheckSpaceWrite (Tag.Length);
            Assert.False(Tag.Length > 255, DNSTagTooLong.Throw);
