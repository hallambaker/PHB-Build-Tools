...
HelpLibraryManagerLauncher.exe /product "VS" /version "100" /locale en-us /uninstall /silent /vendor "Comodo Group Inc" /productName "Goedel Libraries" /mediaBookList "Goedel Portable Support Libraries" > NUL
...
REM build time.
IF EXIST "GoedelPortable.msha" COPY /Y "GoedelPortable.msha" HelpContentSetup.msha
...
REM Uninstall first in case it is already there.  If not, it won't install below.  We'll ignore any error output
REM by redirecting it to NUL.
HelpLibraryManagerLauncher.exe /viewerVersion %1  /locale en-us /wait 0 /operation uninstall /vendor "Comodo Group Inc" /productName "Goedel Libraries" /bookList "Goedel Portable Support Libraries" > NUL

REM Install the new content.
HelpLibraryManagerLauncher.exe /viewerVersion %1  /locale en-us /wait 0 /operation install /sourceUri "%CD%\GoedelPortable.msha"
