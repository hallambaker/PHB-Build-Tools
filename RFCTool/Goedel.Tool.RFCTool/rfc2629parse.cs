...
namespace Goedel.Tool.RFCTool {
    public class Rfc2629Parse {
        ...
        List<Section> MakeSections(section[] sections, int level) {
            if (level > 6) throw new Exception("Levels nested too deeply, maximum is 6.");
        ...
        void AddListBlocks(List<TextBlock> Parent, t t) {
            ...
            //    foreach (string s in t.Text) {
            //        Text += s;
            //        }
            //    P P = new P(Text, t.anchor);
            //    Parent.Add(P);
            //    }
        ...
        void AddFigureBlock(List<TextBlock> Parent, figure figure) {
            if (figure.artwork != null && figure.artwork.Value != null) {
                PRE PRE = new PRE(figure.anchor, figure.artwork.Value);
                Parent.Add (PRE);
                }
            }
